...
from .discrete_helpers import UtilityFunction, unique_rows, PrintLog, acq_max


class DiscreteBayesianOptimization(object):

    def __init__(self, f, keys, space, verbose=1):
        """
        :param f:
            Function to be maximized.

        :param space:
            configuration space, numpy.array of tuples
    ...
    def init(self, init_points):
        ...
        np.random.shuffle(self.space)
        random_point = self.space[:init_points]
        #random_point = [self.space[n] for n in np.random.choice(len(self.space), size=init_points)]

        # Concatenate new random points to possible existing
        # points from self.explore method.
        self.init_points += random_point
    ...
    def maximize(self,
        ...
        #for i in range(n_iter):
            ...
            if self.Y[-1] > y_max:
                max_ei_ratio = (self.Y[-1] - y_max) / y_max
                y_max = self.Y[-1]
            else:
                max_ei_ratio = 0
            # print(max_ei_ratio)
